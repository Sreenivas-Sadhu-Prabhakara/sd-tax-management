...
| **Asset Type** | Corporate Tax Record |
| **Control Record** | Corporate Tax Record Procedure |
...
| POST | `/v1/corporate-tax-record-procedure/initiate` | Initiate |
| GET | `/v1/corporate-tax-record-procedure` | Retrieve (list) |
| GET | `/v1/corporate-tax-record-procedure/{crId}/retrieve` | Retrieve |
| PUT | `/v1/corporate-tax-record-procedure/{crId}/update` | Update |
| PUT | `/v1/corporate-tax-record-procedure/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/corporate-tax-record-procedure/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-tax-management:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-tax-management ./helm -n bian-business-support
```

Exposed through the platform Gateway at path prefix `/sd-tax-management` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
